...
def move_robot(initial_position: str, instructions: str, grid_size: str, robot_scent: [str]) -> str:
    ...
    for instruction in instructions:
        if instruction == 'F':
            ...
            if current_coordinates in robot_scent:  # Ignore instruction as previous robot got lost here.
                continue
            x, y = update_coordinates(orientation, x, y)
            if x < 0 or y < 0 or x > max_x or y > max_y:  # Has the robot fallen off the grid?
